...
DAI_ADD(sof/pipe-dai-capture.m4,
	9, SSP, SPK_SSP_INDEX, SPK_SSP_NAME,
	PIPELINE_SINK_9, 2, FMT,
	ifdef(`ECHO_REF_DEMUX',DMIC_48k_PERIOD_US, SPK_MIC_PERIOD_US), 0, ifdef(`ECHO_REF_DEMUX', DMIC_PIPELINE_48k_CORE_ID, SPK_PLAYBACK_CORE),
	SCHEDULE_TIME_DOMAIN_TIMER)',
`
...
DAI_ADD(sof/pipe-echo-ref-dai-capture.m4,
	...
	SPK_MIC_PERIOD_US, 0, ifdef(`ECHO_REF_DEMUX', DMIC_PIPELINE_48k_CORE_ID, SPK_PLAYBACK_CORE), SCHEDULE_TIME_DOMAIN_TIMER)

`# Capture pipeline 9 from demux on PCM 6 using max 'ifdef(`4CH_PASSTHROUGH', `4', `2')` channels of s32le.'
ifdef(`ECHO_REF_DEMUX',
      `PIPELINE_PCM_ADD(sof/pipe-demux-capture-sched.m4,
	9, 6, ifdef(`4CH_PASSTHROUGH', `4', `2'), s32le,
	SPK_MIC_PERIOD_US, 1, DMIC_PIPELINE_48k_CORE_ID,
	48000, 48000, 48000,
	SCHEDULE_TIME_DOMAIN_TIMER)',
...
`        lines ['
`                # mux to capture'
`                dapm(N_AEC_REF_BUF, PIPELINE_DEMUX_9)'
`       ]'
`}'
dnl else
, `')
...
# Enabled LOOP BACK MODE (LBM) to drive Google AEC via SSP.IN
